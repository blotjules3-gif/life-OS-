...
| F16 | Durcissement backend (2026-07-03) : **rate limiting** /chat + /chat/stream (10/min, 80/h par device_id, 429 FR, fenêtre glissante en mémoire — valide car 1 seul process uvicorn) ; **budget temps agent** (soft 50 s → tools retirés, le LLM conclut en texte ; hard 80 s → clôture propre en gardant les actions exécutées — sous le timeout iOS 90 s) ; **16 index** des migrations 001-004 garantis au démarrage (`CREATE INDEX IF NOT EXISTS` dans le lifespan — create_all n'en crée aucun) ; **course get_or_create_user corrigée** (2 premières requêtes simultanées du même device → UniqueViolation 500, trouvée en testant le 429 en prod ; savepoint + re-select) ; décision : pas d'Alembic complet (sur-ingénierie projet solo, .sql manuels + index idempotents suffisent) ; pytest 14/14, déployé Railway, 429 vérifié en prod | Fait |
| Bloqué | Apple Watch (nouveau target = Xcode GUI) ; défis entre amis (feature à builder) ; capability iCloud (Xcode GUI) ; push GitHub (auth) ; révocation du vieux PAT | En attente Jules |
