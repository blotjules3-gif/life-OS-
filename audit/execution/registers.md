...
| Bloqué | Apple Watch (nouveau target = Xcode GUI) ; défis entre amis + prompts.py améliorés (backend Railway inaccessible) ; capability iCloud (Xcode GUI) ; **note : la rotation de clé B1 est de facto annulée tant que Railway garde l'ancienne — à re-trancher au moment du déploiement** | En attente Jules |
