...
struct WakeUpView: View {
    ...
    @State private var alarmTime: Date = Calendar.current.date(bySettingHour: 7, minute: 0, second: 0, of: .now) ?? .now
        ...
        VStack(spacing: 18) {
            DatePicker("", selection: $alarmTime, displayedComponents: .hourAndMinute)
                ...
                .onChange(of: alarmTime) { _, val in
                    ...
                    if wakeupEnabled { scheduleAlarm() }
                }
    ...
    private func scheduleAlarm() {
        Task {
            ...
            NotificationManager.shared.scheduleDaily(
                id: "lifeos.wakeup",
                title: "Bonjour \(userName.isEmpty ? "" : userName) !",
