...
struct MacroLogView: View {
    ...
    private var kcal: Int { today.reduce(0) { $0 + $1.calories } }
    private var protein: Double { today.reduce(0) { $0 + $1.protein } }
    private var carbs: Double { today.reduce(0) { $0 + $1.carbs } }
    private var fat: Double { today.reduce(0) { $0 + $1.fat } }
                ...
                VStack(spacing: 16) {
                    VStack(spacing: 12) {
                        HStack {
                            ...
                            Spacer()
                            ProgressRing(progress: Double(kcal)/Double(max(1,effectiveKcalGoal)), lineWidth: 10, tint: .nutriTint)
                                .frame(width: 64, height: 64)
                        }
                        HStack(spacing: 10) {
                            macroBar("Protéines", protein, Double(proteinGoal), .red)
                            macroBar("Glucides", carbs, 250, .orange)
                            macroBar("Lipides", fat, 70, .yellow)
                        }
                    }.card()
                    ...
                    if today.isEmpty {
                        ...
                        VStack(spacing: 8) {
                            ForEach(today.sorted(by: { $0.date > $1.date })) { f in
                                HStack {
                                    VStack(alignment: .leading) {
                                        Text(f.name).font(.subheadline.weight(.semibold)).foregroundStyle(Theme.textPrimary)
                                        Text(f.meal).font(.caption).foregroundStyle(Theme.textSecondary)
                                    }
                                    Spacer()
                                    Text("\(f.calories) kcal").font(.subheadline.bold()).foregroundStyle(.nutriTint)
                                    Button(role: .destructive) { ctx.delete(f) } label: { Image(systemName: "trash").font(.caption) }
    ...
    private func macroBar(_ label: String, _ v: Double, _ goal: Double, _ c: Color) -> some View {
        VStack(spacing: 4) {
            Text("\(Int(v))g").font(.subheadline.bold()).foregroundStyle(Theme.textPrimary)
            ProgressView(value: min(1, v/goal)).tint(c)
