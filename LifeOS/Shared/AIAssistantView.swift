...
// MARK: - ImageIntel → extrait dans LifeOS/Services/ImageIntel.swift
...
// MARK: - Server Config Sheet → extrait dans LifeOS/Shared/ServerConfigView.swift
...
//
// Vue liste des conversations passées, groupées par jour. Contenu inline dans
// ce fichier plutôt que dans un fichier séparé : PBXFileSystemSynchronizedRootGroup
// (Xcode 26) rate parfois la détection d'un fichier créé pendant que le projet
// est ouvert. Ici on hérite de l'index existant, plus de risque de "Cannot find
// in scope" au build device.
...
struct CoachHistoryView: View {
    ...
    @Environment(\.dismiss) private var dismiss
    @Environment(\.modelContext) private var historyCtx
